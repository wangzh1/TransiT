...
class fk(nn.Module):
    ...
    def __init__(self, spatial=256, crop=512, \
        ...
        # self.spatial_grid = spatial  yuanlai 
    ...
    def parpareparam(self,):
        ...
        gridz_M = gridz_M / (temprol_grid - 1)  #norm 0 -1 
        gridz_1xMx1x1 = gridz_M.reshape(1, -1, 1, 1)
        self.gridz_1xMx1x1 = torch.from_numpy(gridz_1xMx1x1.astype(np.float32))  #1 256 1 1
    ...
    def forward(self, feture_bxdxtxhxw):
        ...
        datafre_BDX2Tx2Hx2W = torch.fft.fftn(datapad_BDx2Tx2Hx2W)
        real_part = datafre_BDX2Tx2Hx2W.real
        imag_part = datafre_BDX2Tx2Hx2W.imag
        datafre_BDX2Tx2Hx2Wx2 = torch.stack((real_part, imag_part), dim=-1)
        ...
        datafre_BDX2Tx2Hx2Wx2_test = torch.fft.fftshift(datafre_BDX2Tx2Hx2W)
        ...
        tdata_BDx2Tx2Hx2W = torch.view_as_complex(tdata_BDx2Tx2Hx2Wx2)
        data = torch.fft.ifftn(tdata_BDx2Tx2Hx2W)
...
if __name__ == '__main__':
    ...
    test_fk = True
    name = 'person01-02034'
    path = f'/storage/data/yuyh/depthdataset2/data/test/meas/{name}.hdr'
    # path = '/home/yuyh/NLOS_VIDEO/realdata_resize1031.mat'
    if test_fk:
        model = fk(spatial= 256, crop=512, bin_len=0.01, \
                     wall_size=2)
        ...
        p = np.max(re, axis=0)
        p = p/np.max(p)
        cv2.imwrite(f'rebuttal/fk/{name}.png', p*255)
        print('done')
        print(out.shape)
