...
class lct(nn.Module):
    ...
    def todev(self, dev, dnum):
        ...
        self.gridz_1xMx1x1_todev = self.gridz_1xMx1x1.cuda()
        self.datapad_Dx2Tx2Hx2W = torch.zeros((dnum, 2 * self.crop, 2 * self.spatial_grid, 2 * self.spatial_grid)).cuda()
        
        self.mtx_MxM_todev = self.mtx_MxM.cuda()
        self.mtxi_MxM_todev = self.mtxi_MxM.cuda()
        
        self.invpsf_real_todev = self.invpsf_real.cuda()
        self.invpsf_imag_todev = self.invpsf_imag.cuda()
        self.learnanle_w_todev = self.learnanle_w.cuda()
        
        if self.method == 'bp':
            self.lapw_todev = self.lapw.cuda()
    ...
    def forward(self, feture_bxdxtxhxw):
        ...
        # datafre = torch.fft.rfft(datapad_BDx2Tx2Hx2W, 3, onesided=False)
        ...
        right = volumn_BDxTxHxW.reshape(bnum * dnum, temprol_grid, -1).real
        tmp = torch.matmul(left, right)
        tmp2 = tmp.view(bnum * dnum, temprol_grid, sptial_grid, sptial_grid)
